package com.atguigu.tree.threadedbinarttree;
...
/**
 *     线索化后,各个结点指向有变化,因此原来的遍历方式不能使用,这是需要使用新的方式遍历线索化二叉树,各个结点可以通过线型方式遍历,
...
class BinaryTree {
    ...
    public void threadedList(){
        ...
        while (node != null){
            ...
            // 后面随便遍历而变化,因为当leftType == 1时,说明该结点是按照线索化
            // 处理后的有效结点
...
class HeroNode {
    ...
    // 2、如果rightType == 0 表示指向的是右子树,如果1表示指向后续结点
    ...
    public com.atguigu.tree.threadedbinarttree.HeroNode getLeft() {
        return left;
    }
     ...
     * 2、如果删除的节日按是非叶子结点,则删除该子树;
     */
    public void delNode(int no) {
        /**
         * 思路
         * 1.因为我们的二叉树是单向的,所以我们是判断当前结点的子结点是否需要删除结点m，而不能去判断当前这个结点时不时需要删除结点；
         * 2.如果当前结点的左子结点不为空，并且左子结点就是要删除结点，就将this.left = null;并且返回(结束递归删除)
         * 3.如果当前结点的右子节点不为空，并且右子结点就是要删除结点，就将this.right = null;并且就返回(结束递归删除)
