...
// 创建hashTable,管理多条table
class HashTable {
    private EmpLinkedList[] empLinkedListArray;
    private int size; // 表示共有表少条链表
    ...
    public void findEmpById(int id){
        ...
        if (emp != null){ // 找到
            System.out.printf("在第X条链表中找到该雇员 id = %d\n",empLinkedListNo + 1,id);
        }else {
...
class EmpLinkedList {
    // 需要一个头指针
    // 头指针指向第一个雇员Emp,因此我们这个链表的head 是直接指向第一个Emp
    ...
    public void list(int num) {
        if (head == null) { //说明链表为空
            System.out.println("第" + (num+1) + "前链表为空");
            return;
        }
    ...
    public Emp findEmpById(int id){
        ...
        while (true){
            ...
            // 退出条件
            if (curEmp.next == null){ // 说明遍历当前链表,没有找到该雇员
                curEmp = null;
                break;
            }
